...
function spawnTerraformSchema(cwd) {
  return new Promise((resolve5, reject) => __async(this, null, function* () {
    const tf = (0, import_child_process.spawn)("terraform", ["providers", "schema", "-json"], { cwd });
    let data = [];
    tf.stdout.on("data", (chunk) => {
      data.push(chunk);
    });
    tf.stdout.on("close", (chunk) => {
      if (chunk) {
        data.push(chunk);
      }
      resolve5(data.join(""));
...
function getFullProviderName(providerName) {
...
function getLatestVersion(providerName) {
...
function getProviderInformation(providerName, version) {
  return __async(this, null, function* () {
    if (!version) {
      version = yield getLatestVersion(providerName);
    }
...
function extract(_0, _1) {
  return __async(this, arguments, function* (providerName, version, resources = [], dataSources = [], outFile = "") {
    const tfFolder = yield import_promises.default.mkdtemp("tf-");
    try {
      const tfFile = import_path5.default.join(tfFolder, "main.tf");
      const providerInfo = yield getProviderInformation(providerName, version);
    ...
    `,
      ...
      if (outFile) {
        yield import_promises.default.writeFile(outFile, JSON.stringify(trimmedSchema), "utf-8");
      } else {
      ...
      import_promises.default.rm(tfFolder, { recursive: true });
    }
    return {};
  });
}
...
(() => __async(exports, null, function* () {
  const out = yield yargs_default(process.argv.slice(2)).scriptName("schema-extractor").usage("Usage: $0 [options]").options({
    provider: {
      alias: "p",
      type: "string",
      demandOption: true,
      description: "Provider name, e.g. aws. Adds hashicorp if namespace not provided."
    },
    providerVersion: {
      alias: "v",
      type: "string",
